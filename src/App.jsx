...
export default function App() {
  ...
  const handleShareGame = useCallback(async () => {
    ...
    const text = "Can you identify historical figures from just their birth and death locations?";
    if (navigator.share) {
      try {
        await navigator.share({ title: "Lifeguessr", text, url });
      } catch {
        // user cancelled
      }
    } else {
  ...
  return (
    <div className="app">
      ...
      <nav className="app-nav">
        {currentPage === "rules" ? (
          <button onClick={() => setCurrentPage("game")} className="app-nav-btn">
            ← BACK
          </button>
        ) : (
          <>
            <button onClick={() => setCurrentPage("rules")} className="app-nav-btn">
              RULES
            </button>
            <span className="app-nav-sep">◈</span>
            <button onClick={handleShareGame} className="app-nav-btn">
              {gameCopied ? "COPIED" : "SHARE"}
            </button>
          </>
        )}
      </nav>
